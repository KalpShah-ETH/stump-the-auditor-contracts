...
library LendingMath {
    ...
    function updatedReserve(
        ILendingPool.Reserve memory reserve,
        uint256 timestamp
    )
        internal
        pure
        returns (
            ILendingPool.Reserve memory updated,
        ...
        )
    {
        ...
        uint256 totalSupplyActual = scaledToUnderlying(
            updated.totalScaledSupply,
            updated.supplyIndex,
            Math.Rounding.Floor
        );
        uint256 totalBorrowActual = scaledToUnderlying(
            updated.totalScaledBorrow,
            updated.borrowIndex,
            Math.Rounding.Floor
        );
...

        {
            uint256 utilizationRay = totalSupplyActual == 0
                ...
                : totalBorrowActual == 0
                ? 0
                : Math.mulDiv(totalBorrowActual, RAY, totalSupplyActual);
            if (utilizationRay > RAY) utilizationRay = RAY;
            (uint256 borrowRatePerYear, uint256 supplyRatePerYear) = ratesRay(
                updated,
                utilizationRay
            );

            newBorrowIndex = Math.mulDiv(
                oldBorrowIndex,
                RAY + ((borrowRatePerYear / SECONDS_PER_YEAR) * dt),
                RAY
            );
            if (totalSupplyActual != 0) {
                updated.supplyIndex = Math.mulDiv(
                    updated.supplyIndex,
                    RAY + ((supplyRatePerYear * dt) / SECONDS_PER_YEAR),
                    RAY
                );
            }
        }
        ...
        if (
            totalBorrowActual != 0 &&
            updated.reserveFactorBps != 0 &&
            newBorrowIndex > oldBorrowIndex
        ) {
            uint256 borrowDelta = Math.mulDiv(
                totalBorrowActual,
                newBorrowIndex - oldBorrowIndex,
                oldBorrowIndex
            );
            reserveDelta = Math.mulDiv(
                borrowDelta,
                updated.reserveFactorBps,
                BPS
            );
    ...
    {
        uint256 optimalUtilizationRay = Math.mulDiv(
            reserve.irParams.optimalUtilizationBps,
            RAY,
            BPS
        );
        ...
        if (utilizationRay <= optimalUtilizationRay) {
            if (optimalUtilizationRay == 0) {
                ...
                borrowRatePerYear =
                    reserve.irParams.baseRateRayPerYear +
                    Math.mulDiv(
                        reserve.irParams.slope1RayPerYear,
                        utilizationRay,
            ...
            borrowRatePerYear =
                uint256(reserve.irParams.baseRateRayPerYear) +
        ...
        uint256 borrowRevenueRate = Math.mulDiv(
            borrowRatePerYear,
            utilizationRay,
            RAY
        );
        supplyRatePerYear = Math.mulDiv(
            borrowRevenueRate,
    ...
    function utilizationRateRay(
        ILendingPool.Reserve memory reserve
    ) internal pure returns (uint256 utilizationRay) {
        uint256 totalSupplyActual = scaledToUnderlying(
    ...
    function assetValueWad(
        uint256 amount,
        uint8 decimals,
        uint256 price
    ) internal pure returns (uint256 valueWad) {
        if (amount == 0) return 0;
        valueWad = Math.mulDiv(
            toWad(amount, decimals),
            price,
            pow10(ORACLE_DECIMALS)
        );
    }

    function amountFromValueWad(
        uint256 valueWad,
        uint8 decimals,
        uint256 price
    ) internal pure returns (uint256 amount) {
        return
            amountFromValueWad(valueWad, decimals, price, Math.Rounding.Floor);
    }
    ...
    function amountFromValueWad(
        ...
        amount = fromWad(
            Math.mulDiv(valueWad, pow10(ORACLE_DECIMALS), price, rounding),
            decimals,
            rounding
        );
    }

    function scaledToUnderlying(
    ...
    function toWad(
        uint256 amount,
        uint8 decimals
    ) internal pure returns (uint256 wadAmount) {
        if (decimals == 18) return amount;
        if (decimals < 18)
            return Math.mulDiv(amount, pow10(uint8(18 - decimals)), 1);
